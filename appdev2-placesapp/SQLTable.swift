...
class SQLTable: NSObject, SQLTableProtocol {
	...
	func save(updateCloud: Bool = false, dbOverride: DBType = .none) -> Int {
		let key = primaryKey()
		let data = values()
		var insert = true
		if let rid = data[key] {
...
extension SQLTableProtocol where Self: SQLTable {
    ...
    static func rows(filter: String = "", parameters: [Any]? = nil, order: String = "", limit: Int = 0, type: FetchType = .nondeleted) -> [Self] {
        ...
        return self.rowsFor(sql:sql, parameters: parameters)
	}
    ...
    static func rowsFor(sql: String = "", parameters: [Any]? = nil) -> [Self] {
		var res = [Self]()
		let tmp = self.init()
		let data = tmp.values()
		let db = SQLiteDB.shared
		let fsql = sql.isEmpty ? "SELECT * FROM \(table)" : sql
        let arr = db.query(sql:fsql, parameters: parameters)
		for row in arr {
			let t = self.init()
			for (key, _) in data {
				if let val = row[key] {
